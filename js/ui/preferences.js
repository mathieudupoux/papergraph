...
export async function updateUsername() {
  if (!window.currentUser) {
    try {
      const authMod = await import('../auth/auth.js');
      window.currentUser = await authMod.getCurrentUser();
    } catch(e) {}
  }
  ...
  if (!newUsername || newUsername.length < 3 || newUsername.length > 20) { showNotification('Username must be 3-20 characters','error'); return; }
  if (!/^[a-zA-Z0-9_]+$/.test(newUsername)) { showNotification('Username can only contain letters, numbers and underscore','error'); return; }
  try {
    const { data: existingUser } = await supabase.from('user_profiles').select('id').eq('username', newUsername).single();
    if (existingUser && existingUser.id !== currentUser.id) { showNotification('Username already taken','error'); return; }
...
export async function confirmDeleteAccount() {
  ...
  if (check !== 'DELETE') { showNotification('Account deletion cancelled','info'); return; }
  try {
...
export function reportBug() { window.open('https://github.com/remyvallot/papergraph/issues/new','_blank'); }
...
export function initPreferencesFromPage() {
  initOnce();
}
